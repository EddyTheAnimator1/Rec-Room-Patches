...
# "recroom@gmail.com:recnet87"
BASIC_AUTH_HEADER = "Basic cmVjcm9vbUBnbWFpbC5jb206cmVjbmV0ODc="
DEFAULT_BASIC_AUTH_PROFILE_ID = 1
...
def _authorization_header(request: Request | WebSocket) -> str:
    return request.headers.get("Authorization") or request.headers.get("authorization") or ""


def _is_static_basic_auth(request: Request | WebSocket) -> bool:
    return _authorization_header(request).strip().casefold() == BASIC_AUTH_HEADER.casefold()


def _player_id_from_authorization(request: Request | WebSocket) -> int:
    auth = _authorization_header(request)
    match = _BEARER_RE.fullmatch(auth.strip())
    if match:
        return int(match.group(1))
    if auth.strip().casefold() == BASIC_AUTH_HEADER.casefold():
        return DEFAULT_BASIC_AUTH_PROFILE_ID
    return 0
...
# module globals. Retarget them so every delegated 7 January route accepts the
# static Basic auth header and local bearer token too.
...
async def _handle_auth_bootstrap(request: Request, context) -> Response:
    ...
    if not platform_id and _is_static_basic_auth(request):
        platform_id = "7january2017-basic"
    if not platform_id:
