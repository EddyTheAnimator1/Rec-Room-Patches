"""9 November 2016 Rec Room HTTP API adapter.

Confirmed from decompiled client build 860756852849299734:
...
The RecNet HTTP surface and objective parsing match 8 November 2016. The build
DLL itself is different, so this module keeps a dedicated public route and data
scope instead of aliasing another version in BASE.py.
"""
...
def _load_base_adapter():
    module_path = Path(__file__).with_name("8november2016.py")
    spec = importlib.util.spec_from_file_location("recroom_api_8november2016_shared_for_9november2016", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Could not load 8november2016 adapter.")
